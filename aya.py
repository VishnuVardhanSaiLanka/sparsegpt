...
def get_llm(model, token):
    ...
    model = AutoModelForCausalLM.from_pretrained(model, torch_dtype='auto', token=token)
    model.seqlen = 4096 #2048
    return model


@torch.no_grad()
def aya_sequential(model, dataloader, dev):
    print("Starting...")
    ...
    for i in tqdm(range(len(layers))):
        ...
        for names in sequential:
            ...
            for j in range(args.nsamples):
                # print("layer: ", layer)
                # print("________")
                # print("inps: ", inps[j].unsqueeze(0))
                # print("________")
                # print("post: ", layer(inps[j].unsqueeze(0), attention_mask=attention_mask))
...
@torch.no_grad()
def aya_eval(model, testenc, dev,  dataset: str, log_wandb: bool = False):
    print("Evaluating ...")
...
if __name__ == "__main__":
    ...
    if (args.sparsity or args.prunen) and not args.gmp:
        tick = time.time()
        aya_sequential(model, dataloader, DEV)
